 ...
 */
export function checkGates(skillName: string, prompt: string, cwd?: string): GateCheckResult[] {
  const manifest = loadSkillManifest(skillName, cwd);
  if (!manifest || manifest.gates.length === 0) return [];

  const workDir = cwd ?? process.cwd();
  return manifest.gates.map((gate) =>
    evaluateGate(gate.type, gate.description, gate.blocking ?? true, prompt, workDir, gate.pattern)
  );
}
...
function evaluateGate(
  ...
  switch (type) {
    ...
    case 'workflow_not_active': {
      ...
      if (!existsSync(stateFile)) return { passed: true, gate: type, description, blocking };
      try {
        const state = JSON.parse(readFileSync(stateFile, 'utf-8')) as { active?: boolean };
    ...
    case 'custom': {
      if (!pattern) return { passed: true, gate: type, description, blocking };
      try {
