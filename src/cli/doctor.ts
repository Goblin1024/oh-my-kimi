...
function resolveDoctorScope(cwd: string): 'user' | 'project' {
  const scopePath = join(cwd, '.omk', 'setup-scope.json');
  if (!existsSync(scopePath)) return 'user';
  try {
...
function resolveDoctorPaths(cwd: string, scope: 'user' | 'project') {
  if (scope === 'project') {
...
function checkSkills(skillsDir: string): CheckResult[] {
...
function checkAgents(agentsDir: string): CheckResult[] {
  const results: CheckResult[] = [];
  const manifest = tryReadCatalogManifest();
  const expected = manifest?.agents.filter((a) => a.status === 'active') ?? [];
...
function checkHooks(configPath: string): CheckResult {
...
function checkMcpRegistration(kimiHomeDir: string): CheckResult {
