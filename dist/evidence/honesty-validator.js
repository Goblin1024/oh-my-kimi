...
export function validateClaim(claim, evidence) {
    ...
    if (lowerClaim.includes('test') &&
        (lowerClaim.includes('pass') || lowerClaim.includes('all green'))) {
        const ev = evidence.find((e) => e.step === 'tests_passed');
        if (!ev)
            return { valid: false, reason: 'Claimed tests pass but no test evidence submitted' };
        if (ev.exitCode !== 0)
            return { valid: false, reason: `Tests exited with code ${ev.exitCode}` };
    ...
    if (lowerClaim.includes('build') &&
        ...
        if (ev.exitCode !== 0)
            return { valid: false, reason: `Build exited with code ${ev.exitCode}` };
    }
    // Pattern: "lint clean" / "lint passes"
    if (lowerClaim.includes('lint') &&
        (lowerClaim.includes('clean') || lowerClaim.includes('pass'))) {
        const ev = evidence.find((e) => e.step === 'lint_clean');
        if (!ev)
            return { valid: false, reason: 'Claimed lint clean but no lint evidence submitted' };
        if (ev.exitCode !== 0)
            return { valid: false, reason: `Lint exited with code ${ev.exitCode}` };
    }
    // Pattern: "types clean" / "type check passes"
    if ((lowerClaim.includes('type') || lowerClaim.includes('tsc')) &&
    ...
    if (lowerClaim.includes('architect') &&
    ...
    if (lowerClaim.includes('security') &&
        (lowerClaim.includes('review') || lowerClaim.includes('approv'))) {
    ...
    if (lowerClaim.includes('todo') &&
        (lowerClaim.includes('clear') || lowerClaim.includes('done') || lowerClaim.includes('empty'))) {
        const ev = evidence.find((e) => e.step === 'todo_cleared');
        if (!ev)
            return { valid: false, reason: 'Claimed TODO cleared but no TODO evidence submitted' };
    }
    // Pattern: "PRD written" / "spec written"
    if ((lowerClaim.includes('prd') || lowerClaim.includes('spec')) &&
...
//# sourceMappingURL=honesty-validator.js.map