...
describe('agents/definitions', () => {
    ...
    it('each agent has token budget configuration', () => {
        for (const [key, def] of Object.entries(AGENT_DEFINITIONS)) {
            assert.ok(typeof def.tokenBudget === 'number' && def.tokenBudget > 0, `${key} should have a positive tokenBudget`);
            assert.ok(typeof def.maxSteps === 'number' && def.maxSteps > 0, `${key} should have a positive maxSteps`);
