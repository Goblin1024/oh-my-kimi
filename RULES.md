...
- If a blocker is encountered, escalate immediately — do not guess around it.

---

## Part 2 — Code Quality

### R-10 · Follow the Project's Existing Style
...
### R-11 · Names Must Communicate Intent
...
### R-14 · No Magic Numbers or Strings
...
### R-15 · Dead Code Is Deleted, Not Commented Out
...
// Good
// (deleted — see git history if needed)
```
...
- Bug fixes → regression test that fails before the fix, passes after
...
### R-23 · Test Coverage Is a Signal, Not a Goal

100% coverage with trivial tests is worthless. Focus on:

1. **Critical paths** — the code that matters most when it breaks
...
- Simple question → direct answer, no preamble
- Code change → what changed and why, not line-by-line narration
- Error report → error + root cause + fix, not a story
...
Do not loop indefinitely. Escalation is not failure — it is correct behavior.

---

## Part 5 — Safety and Boundaries
...
- If a secret is found in existing code, flag it — do not propagate it
...
### R-62 · Verify Tool Output Before Acting On It
...
```
[ ] All stated requirements are met — verified, not assumed
...
*Rules version: 1.0.0 · Project: oh-my-kimi*
