...
describe('validation/review-delegator', () => {
  ...
  beforeEach(() => {
    testDir = join(
      tmpdir(),
      `omk-review-test-${Date.now()}-${Math.random().toString(36).slice(2)}`
    );
  ...
  function makeEvidence(step: string, reviewerAgent?: string): Evidence {
    return {
      skill: 'ralph',
      step,
      phase: 'verifying',
  ...
  describe('createReviewTasks', () => {
    it('creates tasks for blocking results', () => {
      const blocking: ValidationCheckResult[] = [
        {
  ...
  describe('orchestrateReviews', () => {
    it('delegates tasks within slot limits', () => {
      ...
      const { delegated, queued } = orchestrateReviews(
        blocking,
        makeEvidence('impl'),
        mailboxPath,
        2
