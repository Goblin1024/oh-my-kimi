...
import { getTeamState, setTeamState, updateWorkerState, TeamState, WorkerState } from './state.js';
...
export class TeamRuntime {
  ...
  async startTeam(count: number, role: string, task: string, cwd?: string): Promise<void> {
  ...
  private spawnWorker(workerState: WorkerState, cwd?: string): void {
    ...
    child.on('error', (err) => {
      if ((err as NodeJS.ErrnoException).code === 'ENOENT') {
        console.error(
          `\x1b[31m[${workerId}]\x1b[0m Error: 'kimi' command not found in PATH. Please install Kimi CLI to use team mode.`
        );
      } else {
    ...
    child.on('exit', (code, signal) => {
      logStream.end();
      this.workers.delete(workerId);
      let status: 'completed' | 'failed' | 'terminated';
  ...
  private checkTeamCompletion(cwd?: string): void {
  ...
  async shutdownTeam(cwd?: string): Promise<void> {
    const state = getTeamState(cwd);
