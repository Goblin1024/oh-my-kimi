...
export async function uninstall() {
    ...
    if (existsSync(KIMI_CONFIG)) {
        ...
        try {
            ...
            if (config.hooks && Array.isArray(config.hooks)) {
                ...
                config.hooks = config.hooks.filter((h) => !(h.command && typeof h.command === 'string' && h.command.includes('omk')));
    ...
    if (existsSync(OMK_SKILLS_DIR)) {
        try {
            ...
            console.error(`  ✗ Failed to remove skills dir: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    else {
