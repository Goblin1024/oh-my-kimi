...
function parseCountAndRole(arg) {
...
export async function teamCommand(args) {
    ...
    if (subCommand === 'status') {
        ...
        console.log(`Team Session: ${state.active ? '\x1b[32mACTIVE\x1b[0m' : '\x1b[31mINACTIVE\x1b[0m'}`);
        ...
        for (const w of state.workers) {
            ...
            if (w.status === 'failed' || w.status === 'terminated')
                color = '\x1b[31m'; // red
            console.log(`  [${w.id}] PID:${w.pid || '---'} Status: ${color}${w.status.padEnd(10)}\x1b[0m Task: ${w.task.substring(0, 50).replace(/\n/g, ' ')}...`);
        }
        return;
    }
