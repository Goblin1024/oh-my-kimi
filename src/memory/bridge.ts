...
export interface StatusResult {
  ...
  wings: Array<{
    name: string;
    rooms: Array<{ name: string; drawers: number }>;
  }>;
}
...
export interface MemPalaceBridge {
  ...
  /** Show palace status */
  status(options?: StatusOptions): Promise<CommandResult & { status?: StatusResult }>;
}
...
export interface StatusOptions {
  // No extra options for status at this time
}
...
function getOmkDir(projectRoot?: string): string {
  return join(projectRoot || process.cwd(), '.omk');
}

function getPalacePathFile(projectRoot?: string): string {
 ...
 */
export function getPalacePath(projectRoot?: string): string {
  const file = getPalacePathFile(projectRoot);
  if (existsSync(file)) {
    try {
      const cached = readFileSync(file, 'utf-8').trim();
      if (cached) return resolve(projectRoot || process.cwd(), cached);
    } catch {
 ...
 */
export function setPalacePath(palacePath: string, projectRoot?: string): void {
...
function buildArgs(
  ...
  mappers: Record<string, (value: unknown) => string[]>
): string[] {
  const args = [...base];
 ...
 *   ========================================================
 */
export function parseStatusOutput(stdout: string): StatusResult | undefined {
  ...
  const wings: StatusResult['wings'] = [];

  const wingRe = /WING:\s*(.+)/g;
  const roomRe = /ROOM:\s*(\S+)\s+(\d+) drawers/g;

  let wingMatch: RegExpExecArray | null;
  while ((wingMatch = wingRe.exec(stdout)) !== null) {
    ...
    const rooms: Array<{ name: string; drawers: number }> = [];
 ...
 *   <L0 + L1 text here>
 */
export function parseWakeUpOutput(stdout: string): string {
 ...
 */
export async function createBridge(projectRoot?: string): Promise<MemPalaceBridge> {
  ...
  return {
    ...
    async status(_options = {}) {
      if (!available) return { ...(await fallback('status')) };
      const args = ['status'];
      if (!args.includes('--palace')) {
        args.push('--palace', palacePath);
      }
 ...
 */
export async function addToMemPalace(
  content: string,
  metadata: { wing: string; room: string; tags?: string[] }
): Promise<void> {
  const available = await isMemPalaceAvailable();
  if (!available) {
  ...
  try {
    ...
    if (!result.success) {
      ...
      try {
        const parsed = JSON.parse(result.stdout.trim()) as { success: boolean; error?: string };
        if (!parsed.success) {
 ...
 */
export async function getMemPalaceStatus(): Promise<PalaceStatus> {
  const available = await isMemPalaceAvailable();
  if (!available) {
 ...
 */
export async function wakeUpMemPalace(): Promise<string> {
  const available = await isMemPalaceAvailable();
  if (!available) {
