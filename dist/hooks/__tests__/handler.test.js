...
describe('hooks/handler', () => {
    ...
    let handlerPath;
    beforeEach(() => {
        testDir = join(tmpdir(), `omk-handler-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    ...
    describe('UserPromptSubmit — $cancel', () => {
        it('cancels an active workflow', () => {
            ...
            assert.equal(hookOutput.activated, true);
            assert.ok(hookOutput.message.includes('Cancelled'));
        });
    ...
    describe('SessionStart', () => {
        it('reports active workflow on session start', () => {
            ...
            const hookOutput = output.hookSpecificOutput;
            assert.equal(hookOutput.skill, 'ralph');
            assert.ok(hookOutput.message.includes('Resuming'));
        });
        it('reports no active workflow when none exists', () => {
            ...
            const hookOutput = output.hookSpecificOutput;
            assert.ok(hookOutput.message.includes('No active'));
        });
    });
    describe('Stop', () => {
        it('blocks stop when workflow is active', () => {
            ...
            const hookOutput = output.hookSpecificOutput;
            assert.equal(hookOutput.skill, 'ralph');
            assert.ok(hookOutput.message.includes('active'));
        });
