...
- Reference is a live URL (not a static screenshot â€?use `$visual-verdict` for screenshot-only tasks)
</Use_When>

<Do_Not_Use_When>
- User only has screenshot references without a live URL â€?use `$visual-verdict` directly
- User wants to modify, redesign, or "improve" the site â€?use standard implementation flow
- Target requires authentication, payment flows, or backend API parity â€?out of scope for v1
- Multi-page / multi-route deep cloning â€?v1 handles single-page scope only
</Do_Not_Use_When>
...
- Use `browser_snapshot` (accessibility tree) for structural understanding â€?it is far more token-efficient than screenshots.
- Use `browser_take_screenshot` only when visual verification is needed (Pass 1 baseline, Pass 4 comparison).
- Use `browser_evaluate` for DOM/style extraction â€?pass the scripts from this skill EXACTLY as written (do not modify them).
...
- **When running within ralph**: Also persist the `visual` portion of the composite verdict to `.omk/state/{scope}/ralph-progress.json` for ralph compatibility, mapping `visual.score` â†?top-level `score` and `visual.verdict` â†?top-level `verdict`.
...
3. **Accessibility snapshot**: `browser_snapshot` â€?captures the semantic tree (roles, names, values, interactive states). This is your primary structural reference.
4. **Full-page screenshot**: `browser_take_screenshot` with `fullPage: true` â€?save as reference baseline `target-full.png`.
5. **DOM + computed styles**: `browser_evaluate` with the following script. **COPY THIS SCRIPT EXACTLY â€?do not modify it**:
   ```javascript
   (() => {
   ...
   ```
6. **Interactive elements**: `browser_evaluate` to catalog all interactable elements. **COPY THIS SCRIPT EXACTLY â€?do not modify it**:
   ```javascript
   (() => {
     const results = [];
   ...
   ```
7. **Network patterns** (optional): `browser_network_requests` â€?note XHR/fetch calls for reference. Do not attempt to replicate backends.
...
3. **Create interaction map**: From interactive elements list:
   - Navigation links â†?anchor tags with `href`
   - Form elements â†?proper `<form>` with inputs, labels, validation
   - Buttons â†?click handlers (toggle, submit, navigate)
   - Dropdowns/modals â†?show/hide toggle with transitions
   - Accordions/tabs â†?state-based visibility
...
5. **Define file structure**:
   ```
   {output_dir}/
   â”œâ”€â”€ index.html          (or App.tsx / App.vue)
   â”œâ”€â”€ styles/
   â”?  â”œâ”€â”€ globals.css      (reset + tokens)
   â”?  â””â”€â”€ components.css   (or scoped styles)
   â”œâ”€â”€ scripts/
   â”?  â””â”€â”€ interactions.js  (toggle, modal, dropdown logic)
   â””â”€â”€ assets/              (placeholder images)
   ```
   Adapt to `tech_stack` if specified (React components, Vue SFCs, etc.).

## Pass 3 â€?Generate Clone
...
4. **Components**: Implement each region top-down:
   ...
   - Apply computed styles â€?prioritize layout properties, then typography, then decorative
...
4. **Functional spot-check**: Test 2â€? detected interactions via Playwright:
   - Click a navigation link â†?verify URL change or scroll behavior
   - Toggle a dropdown/modal â†?verify visibility change
   - Interact with a form field â†?verify it accepts input
...
**Pass 2**: Regions: nav bar (logo + links), story table (30 rows Ã— title + meta), footer. Tokens: orange #ff6600, gray #828282, Verdana font, 10pt base. Interaction map: story links (external), comment links, "more" pagination.
...
**Pass 5 iteration 1**: Fix font to Verdana 10pt, increase row padding â†?score=88. Functional 2/2. Structure match. â†?`overall_verdict: pass`. Done.
</Example>
