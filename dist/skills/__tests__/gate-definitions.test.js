...
import { checkShortcutKeywords, checkVerificationPlan, checkDecomposition, checkFlagSemantic, runSemanticGates, hasBlockingGates, formatGateResults, } from '../gate-definitions.js';
describe('skills/gate-definitions', () => {
    ...
    describe('checkDecomposition', () => {
        ...
        it('passes for complex tasks with decomposition indicators', () => {
            const result = checkDecomposition('First refactor the database layer, then update the API, and finally write tests');
            assert.equal(result.passed, true);
        });
        it('fails for complex tasks without clear decomposition', () => {
            const result = checkDecomposition('Design a complete microservices architecture from scratch');
            assert.equal(result.passed, false);
    ...
    describe('checkFlagSemantic', () => {
        ...
        it('warns about --eco on complex tasks', () => {
            const result = checkFlagSemantic('Design distributed system architecture', ['--eco'], 'ralph');
            assert.equal(result.passed, false);
            assert.ok(result.details?.includes('eco'));
        });
        it('passes when flags match complexity', () => {
            const result = checkFlagSemantic('Design distributed system architecture', ['--deliberate'], 'ralph');
            assert.equal(result.passed, true);
        });
    });
...
//# sourceMappingURL=gate-definitions.test.js.map