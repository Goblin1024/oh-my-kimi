...
function readPersistedScope(projectRoot: string): SetupScope | undefined {
  ...
  try {
    ...
    if (raw.scope === 'user' || raw.scope === 'project') return raw.scope;
  } catch {
    // ignore invalid
  }
  return undefined;
...
function persistScope(projectRoot: string, scope: SetupScope): void {
...
export async function setup(scopeArg?: string): Promise<void> {
  ...
  const promptSummary: SetupSummary = { updated: 0, unchanged: 0, skipped: 0 };
  ...
  console.log(
    `  ✓ Prompts: updated=${promptSummary.updated}, unchanged=${promptSummary.unchanged}, skipped=${promptSummary.skipped}\n`
  );

  // Step 3: Install skills (active skills)
  const skillSummary: SetupSummary = { updated: 0, unchanged: 0, skipped: 0 };
  ...
  console.log(
    `  ✓ Skills: updated=${skillSummary.updated}, unchanged=${skillSummary.unchanged}, skipped=${skillSummary.skipped}\n`
  );

  // Step 4: Generate native agent TOMLs
  const agentSummary: SetupSummary = { updated: 0, unchanged: 0, skipped: 0 };
  ...
  console.log(
    `  ✓ Agents: updated=${agentSummary.updated}, unchanged=${agentSummary.unchanged}, skipped=${agentSummary.skipped}\n`
  );
...
function configureMcpServers(kimiHomeDir: string, packageRoot: string): void {
...
function configureHooks(configPath: string, skillsDir: string, packageRoot: string): void {
  ...
  if (existsSync(configPath)) {
    try {
      config = TOML.parse(readFileSync(configPath, 'utf-8')) as KimiConfig;
    } catch (_e) {
  ...
  writeFileSync(configPath, TOML.stringify(config as unknown as any));
