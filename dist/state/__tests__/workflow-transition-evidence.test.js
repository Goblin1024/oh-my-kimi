...
import { assertValidTransition, IllegalStateTransitionError, TransitionBlockedError, } from '../workflow-transition.js';
import { submitEvidence } from '../evidence.js';
describe('state/workflow-transition evidence lock', () => {
    let testDir;
    beforeEach(() => {
        testDir = join(tmpdir(), `omk-transition-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    ...
    function makeEvidence(skill, step, phase) {
        return {
            skill,
            step,
            phase,
...
//# sourceMappingURL=workflow-transition-evidence.test.js.map