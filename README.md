...
[English](./README.md) ‚Ä?[ÁÆÄ‰Ωì‰∏≠Êñá](./README.zh-CN.md) ‚Ä?[Documentation](docs/GETTING-STARTED.md)
...
### üåü What Makes OMK Different

Unlike plain prompt templates, OMK is a **code-level workflow engine**:

- üõ°Ô∏?**Code-Enforced Gates:** Flags and preconditions are validated by code, not just documented.
- üîí **Atomic State Management:** Concurrent-safe file operations with spin-locks prevent state corruption.
- üì° **Event-Driven HUD:** Real-time terminal dashboard using `fs.watch` instead of wasteful polling.
- üîç **Semantic Memory:** BM25-powered memory retrieval instead of naive string matching.
- üß© **Dynamic Skill Discovery:** Skills are parsed from YAML frontmatter at runtime‚Äîno hardcoding required.
...
## üõ†Ô∏?Built-in Skills

| Command | Description | Best Used When... |
| :--- | :--- | :--- |
| üïµÔ∏è‚Äç‚ôÇÔ∏?`$deep-interview` | Socratic requirements gathering | The feature is vague, or boundaries need clarifying. |
| üìê `$ralplan` | Architecture planning & approval | You need a solid, reviewed plan before coding starts. |
| üèÉ‚Äç‚ôÇÔ∏?`$ralph` | Persistence loop to completion | It's time to write code, test, and verify against the plan. |
| üë• `$team` | Parallel multi-agent execution | A task can be broken into independent sub-tasks. |
| üõë `$cancel` | Graceful workflow abort | You need to stop the current agentic process. |

---

## üèóÔ∏?Architecture & Core Features
...
- `src/state/atomic.ts` ‚Ä?`writeAtomic()` + `withFileLock()` (spin-lock with stale detection)
- `src/team/state.ts` ‚Ä?`updateWorkerState()` serializes concurrent worker exit events
...
- **TokenBudget** (`src/token/budget.ts`) ‚Ä?warning/critical/exceeded thresholds; flag multipliers (`--eco` 25%, `--quick` 50%, `--deliberate` 400%)
- **Complexity router** (`src/token/router.ts`) ‚Ä?routes prompts to low/medium/high configs
- **Evidence pruner** (`src/token/pruner.ts`) ‚Ä?compresses evidence >5KB, reclaims tokens after prune
- **SessionAuditor** (`src/token/audit.ts`) ‚Ä?unified budget + route + pruning with audit reports
- **Token HUD panel** ‚Ä?real-time progress bar, remaining tokens, efficiency score
...
- **Slot manager** (`src/team/slot-manager.ts`) ‚Ä?reads `max_running_tasks` from `~/.kimi/config.toml` (default 4)
- **Mailbox** (`src/team/mailbox.ts`) ‚Ä?file-based JSONL messaging between workers
- **KimiRuntime** (`src/team/kimi-runtime.ts`) ‚Ä?spawns real `kimi` processes with heartbeat + auto-restart (max 3)
...
- `prompt_specificity` ‚Ä?blocks vague prompts
- `has_active_plan` ‚Ä?blocks execution without an approved plan
- `workflow_not_active` ‚Ä?prevents concurrent workflow collisions
- `custom` ‚Ä?regex-based predicate matching
- **Semantic gates** (non-blocking warnings):
  - `no_shortcut_keywords` ‚Ä?detects "just", "simply", "quickly", "hack"
  - `has_verification_plan` ‚Ä?requires test/verify/validate keywords
  - `proper_decomposition` ‚Ä?complex tasks need step markers
  - `flag_semantic_check` ‚Ä?warns on mismatched flags
...
- `src/utils/bm25.ts` ‚Ä?lightweight BM25 with TF-IDF scoring
...
- **`omk mcp state`** ‚Ä?`omk_read_state`, `omk_write_state` (validates transitions), `omk_list_skills`, plus 4 evidence tools (`omk_submit_evidence`, `omk_list_required_evidence`, `omk_verify_evidence`, `omk_assert_phase`)
- **`omk mcp memory`** ‚Ä?`omk_memory_store`, `omk_memory_query` (BM25-ranked), `omk_memory_list`

### 12. Structured Observability

- **`src/utils/logger.ts`** ‚Ä?Hierarchical logging (debug/info/warn/error) writes to `.omk/logs/system.log`
- **`src/utils/audit.ts`** ‚Ä?Hook execution audit (JSONL, daily rotation, 5MB limit)
...
*   üí° [Real-World Examples](docs/EXAMPLES.md)
*   üèóÔ∏?[Architecture Deep Dive](docs/ARCHITECTURE.md)
*   ü§ñ [Agent System Guidance](docs/AGENTS.md)
*   ‚ú?[Verification & Testing](VERIFICATION.md)
...
<div align="center">
  <i>Made with ‚ù§Ô∏è for the Kimi CLI community</i><br>
