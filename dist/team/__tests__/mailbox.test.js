...
import { appendMessage, readMessages, readUndeliveredFor, markDelivered, rewriteMailbox, getMailboxPath, sendTextMessage, } from '../mailbox.js';
import { appendFileSync } from 'fs';
describe('team/mailbox', () => {
    let testDir;
    let mailboxPath;
    beforeEach(() => {
        testDir = join(tmpdir(), `omk-mailbox-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    ...
    function makeMessage(id, to, delivered = false) {
        return {
            id,
            from: 'leader',
            to,
    ...
    describe('sendTextMessage', () => {
        it('sends a simple text message', () => {
            ...
            assert.equal(messages[0].payload.body, 'do the thing');
        });
    });
});
